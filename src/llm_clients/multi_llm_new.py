...
class EnhancedMultiLLMClient(BaseLLMClient):
    ...
    def call_qiniu_api(self, messages: List[Dict], model: str = None, options: Dict = None, max_retries: int = 3) -> Tuple[str, Optional[Dict]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_error = str(e)
                logger.error(f"❌ Qiniu DeepSeek API error on attempt {attempt + 1}: {e}")
                if attempt < max_retries - 1:
                    time.sleep(self.retry_delays[attempt] if attempt < len(self.retry_delays) else 2)
                    continue

        response_time = time.time() - start_time
        self._update_api_stats(endpoint_name, False, response_time, last_error)
        return f"[API Error: Qiniu DeepSeek API failed - {last_error}]", None

    def call_together_api(self, messages: List[Dict], model: str = None, options: Dict = None, max_retries: int = 3) -> Tuple[str, Optional[Dict]]:
        endpoint_name = 'together'
        endpoint_config = self.api_endpoints[endpoint_name]

        if not endpoint_config['key']:
            error_msg = "[API Error: Together API key not configured]"
            self._update_api_stats(endpoint_name, False, 0, error_msg)
            return error_msg, None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if content and content.strip():
                    response_time = time.time() - start_time
                    self._update_api_stats(endpoint_name, True, response_time)
                    logger.info(f"✅ Together success: {len(content)} chars in {response_time:.2f}s")
                    return content, data['choices'][0]['message']
                else:
                    logger.warning(f"⚠️ Empty response from Together on attempt {attempt + 1}")
                    last_error = "Empty response"
                    
            except Exception as e:
                last_error = str(e)
                logger.error(f"❌ Together API error on attempt {attempt + 1}: {e}")
                if attempt < max_retries - 1:
                    time.sleep(self.retry_delays[attempt] if attempt < len(self.retry_delays) else 2)
                    continue
        ...
        response_time = time.time() - start_time
        self._update_api_stats(endpoint_name, False, response_time, last_error)
        return f"[API Error: Together API failed - {last_error}]", None
    ...
    def call_openrouter_api(self, messages: List[Dict], model: str = None, options: Dict = None, max_retries: int = 3) -> Tuple[str, Optional[Dict]]:
        ...
        if not endpoint_config['key']:
            error_msg = "[API Error: OpenRouter API key not configured]"
            self._update_api_stats(endpoint_name, False, 0, error_msg)
            return error_msg, None
        ...
        for attempt in range(max_retries):
            try:
                ...
                if content and content.strip():
                    response_time = time.time() - start_time
                    self._update_api_stats(endpoint_name, True, response_time)
                    logger.info(f"✅ OpenRouter success: {len(content)} chars in {response_time:.2f}s")
                    return content, data['choices'][0]['message']
                else:
                    logger.warning(f"⚠️ Empty response from OpenRouter on attempt {attempt + 1}")
                    last_error = "Empty response"
        ...
        response_time = time.time() - start_time
        self._update_api_stats(endpoint_name, False, response_time, last_error)
        return f"[API Error: OpenRouter API failed - {last_error}]", None
    ...
    def _call_with_load_balancing(self, messages: List[Dict], model: str = None, options: Dict = None) -> Tuple[str, Optional[Dict]]:
        ...
        for i, (api_name, api_func) in enumerate(api_funcs):
            ...
            logger.info(f"Trying {api_name} API ({i+1}/{len(api_funcs)})...")
    ...
    def _call_sequential(self, messages: List[Dict], model: str = None, options: Dict = None) -> Tuple[str, Optional[Dict]]:
        ...
        return "[API Error: No healthy APIs available]", None

    def _get_healthy_apis_sorted(self) -> List[str]:
        healthy_apis = []
        for endpoint, config in self.api_endpoints.items():
            if config.get('key') and self.api_stats[endpoint]['is_healthy']:
                healthy_apis.append((endpoint, config['priority']))
        
        # 按优先级排序
        healthy_apis.sort(key=lambda x: x[1])
        return [endpoint for endpoint, _ in healthy_apis]
    
    def chat_completion(self, messages: List[Dict], model: str = "auto", options: Optional[Dict] = None) -> Dict:
        if options is None:
            options = {}
        
        logger.info(f"EnhancedMultiLLMClient: Processing with model '{model}'")
        
        # 解析模型名称
        if model == "auto":
            # 自动模式：依次尝试所有API
            content, response_message = self._call_with_load_balancing(messages, None, options)
            ...
            # 默认使用自动模式
            content, response_message = self._call_with_load_balancing(messages, model, options)
    ...
    def get_available_models(self) -> List[str]:
        ...
        for endpoint, config in self.api_endpoints.items():
            if config.get('key'):
                models.append(f"{endpoint}/{self.default_models[endpoint]}")
        return models
